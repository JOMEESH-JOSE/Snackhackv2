...
if (isset($_POST['btn'])) {
  ...
  $pin = $_POST['pin'];
  $email = $_POST['email'];
  $pfile = basename($_FILES["file2"]["name"]);
  $targetDir = "image/";
  ...
  if (mysqli_num_rows($valid) > 0) {
    ...
    $sq = "INSERT INTO `login_tb`(`username`, `password`, `role`, `status`) VALUES ('$uname','$pswd','$role','Notactive')";
    if (mysqli_query($conn, $sq)) {
      $roleID = mysqli_insert_id($conn);
      $reg = "INSERT INTO `registraion_tb`(`Lg_id`, `name`, `gender`, `dob`,`image`,`address`, `district_id`, `city_id`, `pin`, `email`,`adhar`, `phno`) VALUES ('$roleID','$name','$gender','$dob','$image','$address','$district','$city','$pin','$email','$pfile','$phno')";
      mysqli_query($conn, $reg);
      echo "<script>alert(' registeration successfully')</script>";
      header("location:verireg.php?eml=$email");
      // echo "<script>location.replace('verifymailreg.php?eml=$email');</script>";
    ...
    <main class="app-content" style="margin-top:10px">
      <div class="app-title">
        ...
        <form class="forget-form" name="myform" method="post" onsubmit="return validate()" enctype="multipart/form-data">
          ...
          <h3 class="login-head" style="margin-top: 25px;"><i class="fa fa-lg fa-fw fa-user"></i>Staff Registration</h3>
          <div style="margin-top: 25px;">
            ...
            <div class="form-group">
              ...
              <input class="form-control" type="file" name="file2" placeholder="enter adharcard image" accept="image/png, image/gif, image/jpeg">
