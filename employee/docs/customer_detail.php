...
include 'db.php';
$lg = $_GET['lg'];
$ww = mysqli_query($conn,"SELECT * FROM user_registration_tb inner join `login_tb` on login_tb.Lg_id = `user_registration_tb`.`login-id` join city on city.id = user_registration_tb.city join district on district.id = user_registration_tb.District Where login_tb.Lg_id ='$lg'");
?>
      ...
      <div class="tab-pane active" id="user-timeline">
       ...
       <tr><?php if ($row['status'] == 'Blocked') { ?>
                            <td><a href="cust_usersts.php?sid=<?php echo $row['Lg_id']; ?>&&sts=1&&eml=<?php echo $row['email']; ?>"><button class="btn btn-primary">Enable</button></a></td>
                        <?php } else { ?>
                            <td><a href="cust_usersts.php?sid=<?php echo $row['Lg_id']; ?>&&sts=2&&eml=<?php echo $row['email']; ?>"><button class="btn btn-primary">Disable</button></a></td>
                            </tr
                        <?php } ?>
                        <?php } ?>
                </thead>
